# =============================================================================
# PowerShell Build Script for Flet Application (Based on Proven .spec Logic)
#
# Описание:
#   Использует проверенную логику для сборки, найденную в рабочем проекте.
#   Генерирует .spec файл, явно прописывая все зависимости.
#
# ИСПОЛЬЗОВАНИЕ:
...
$MainScript = "stock.adobe_parser_gui.py" # Убедитесь, что это ваш главный файл
$IconFile = ""

# --- НАЧАЛО ПРОЦЕССА СБОРКИ ---

Clear-Host
Write-Host "🚀 Начинаю сборку для '$AppName' по проверенной методике..." -ForegroundColor Cyan
...
# Списки зависимостей, как в вашем примере
$addData = @(
    "('ImageParser.py', '.'),",
    "('config.py', '.'),",
    "('utils.py', '.'),",
